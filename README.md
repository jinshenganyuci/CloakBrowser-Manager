...
- 汉化限于管理器的操作、创建和编辑界面。新配置的浏览器语言、时区与原版一样默认留空，不会因为界面是中文就写入 `zh-CN` / `Asia/Shanghai`，也不会屏蔽 GeoIP 自动检测。已有配置的值原样保留。
- 另行增加的地区下拉菜单支持美国、日本、德国等，可自定义或留空；仅在手动选择后按相应配置启动远程浏览器。
- Docker 保留 UTF-8 和中文 / Emoji 字体，以正常显示输入内容；不设置全局中文语言、不强制 KeePassXC 的界面语言，也不翻译远程浏览器默认书签。
...
| 时区、浏览器语言 | 控制远程浏览器的时区、网页语言和浏览器菜单/设置页语言；保存后重启该配置生效。管理器界面固定使用简体中文；KeePassXC 保持其自身设置 |
