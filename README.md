...
- 新建配置默认使用 `zh-CN` 和 `Asia/Shanghai`，时区和语言提供常用地区下拉菜单，包括美国（东部/中部/山地/西部）、日本、德国、英国、法国、韩国等；也可选“自定义”输入其他有效值，或选“留空”交给 GeoIP / 浏览器默认值。语言与时区独立设置，保存后重启该浏览器配置生效。已有配置不会被改成中文语言或中国时区。
...
| 时区、浏览器语言 | 控制远程浏览器的时区、网页语言和浏览器菜单/设置页语言；保存后重启该配置生效。管理器与 KeePassXC 界面固定使用简体中文 |
